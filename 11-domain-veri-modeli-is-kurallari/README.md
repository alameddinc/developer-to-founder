...
Bu haftanın amacı akademik bir "Domain Driven Design (DDD)" dersi vermek değil. Amacımız; MVP heyecanıyla yapılan **"Ekran Odaklı Veritabanı Tasarımı"** hatasını engellemektir. Çünkü yanlış veri modeli, projenin büyümesini durduran görünmez bir duvardır.
...
> _"Profil sayfasında kullanıcının 'Son İzledikleri' var. O zaman Users tablosuna `last_watched_videos` diye bir string array açayım."_
...
Bir kural: _"Kullanıcı kredisi bitmişse 'Yükle' butonu pasif olsun."_
...
* `Table: Videos` -> {id, user\_id, status, file\_url, detected\_silences\_json}
